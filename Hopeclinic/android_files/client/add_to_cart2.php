...
if ($_SERVER['REQUEST_METHOD'] =='POST'){
    ...
    $select="SELECT stock FROM services WHERE stock_id='$productID'";
...
// check if item exist in client shopping cart
        ...
        if($get=mysqli_num_rows($records) >0){
            $response["status"] = 0;
            $response["message"] ='You have Already Booked for this service';
            echo json_encode($response);
        }else{
            ...
            if ($row=mysqli_num_rows($record) < 1 ){
                ...
                $insert="INSERT INTO bookings(client_id)VALUES ('$clientID')";
             ...
             VALUES ('$clientID','$orderNo', '$productID', '$qty')";
                ...
                $select="SELECT order_id,order_status FROM bookings WHERE client_id='$clientID' AND order_status='0'";
                $record=mysqli_query($con,$select);
